...
import { Button } from "@/components/ui/button";
import { Server, Database, Cpu, ArrowRight } from "lucide-react";
import { ServiceOffer } from "@/lib/types"; // <-- important
...
const PlanCard: React.FC<PlanCardProps> = ({ plan, className = "" }) => {
  const PlanIcon = plan.type.toUpperCase().includes("VPS") ? Server : Database;
      ...
      <div className="p-6">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm">{plan.ramGb} GB RAM</span>
          </div>

          <div className="flex items-center space-x-2">
            <Database className="h-4 w-4 text-gray-600" />
            <span className="text-sm">{plan.ssdStorageGb} GB SSD Storage</span>
          </div>
        </div>
      </div>
